...
export default function NotesSettingsPage() {
  ...
  const loadLists = useCallback(async () => {
    ...
    if (error) {
      setErrorMessage("Impossible de charger les listes de notes.");
      setIsLoading(false);
      return;
    }
  ...
  const handleDeleteSelectedList = async () => {
    if (!family || !selectedListId || !userId) {
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-rose-50 to-slate-100">
      ...
      <div className="max-w-5xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-12 space-y-6 sm:space-y-8">
        <div className="bg-white rounded-lg shadow-md p-6 sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <Link
              ...
              className="inline-flex bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-2 rounded font-medium transition-colors"
            >
              Retour aux notes
            </Link>
          </div>
          {message ? <p className="mt-3 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded p-3">{message}</p> : null}
          {errorMessage ? <p className="mt-3 text-sm text-rose-700 bg-rose-50 border border-rose-200 rounded p-3">{errorMessage}</p> : null}
        </div>

        <div className="bg-white rounded-lg shadow-md p-6 sm:p-8 space-y-4">
          <div className="space-y-2">
            <label htmlFor="notes-settings-list-select" className="block text-sm font-medium text-slate-700">
              Choisir une liste
            </label>
            <select
              id="notes-settings-list-select"
              value={selectedListId ?? ""}
              onChange={(event) => setSelectedListId(event.target.value || null)}
              disabled={lists.length === 0 || !family || isLoading}
        ...
        <form onSubmit={handleCreateList} className="bg-white rounded-lg shadow-md p-6 sm:p-8 space-y-3">
